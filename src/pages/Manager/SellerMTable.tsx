...
import SellerMform from './SellerMform';
...
interface Manager {
  ...
  salespersons: number;
  status: boolean;
  todaysTarget: number; // Added field from the reference table
}

const SellerMTable: React.FC = () => {
  ...
  const fetchSellers = (page: number, limit: number) => {
    ...
    axios
      .get(
        `${Urls.sellerMUrl}?page=${page}&limit=${limit}&managerId=${currentUser._id}`,
        {
          headers: {
  ...
  }, [currentPage, itemsPerPage, currentUser._id]);
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              ...
              >
                Total Target {renderSortIcon('salespersons')}
              </th>
              <th
                className="min-w-[120px] py-4 px-4 font-medium text-black dark:text-white cursor-pointer"
