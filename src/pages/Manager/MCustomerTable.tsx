...
interface CustomerTableProps {
  sellerId: string; // Only expect sellerId, not the full Manager object
}
...
const MCustomerTable: React.FC<CustomerTableProps> = ({ sellerId }) => {
  ...
  const fetchSellers = (page: number, limit: number) => {
    ...
    axios
      .get(
        `${Urls.TicketsUrl}?page=${page}&limit=${limit}&sellerId=${sellerId}`,
        {
          headers: {
      ...
      .then((response) => {
        ...
        if (
          ...
          const ticketData = response.data.data.tickets.map((ticket: any) => ({
            ...
            gameUserName: ticket.gameUserId.firstName,
