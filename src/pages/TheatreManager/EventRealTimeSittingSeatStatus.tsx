...
const EventRealTimeSittingSeatStatus: React.FC = () => {
    const { id } = useParams();
...
const { booked, available } = getSeatCounts();
  ...
  useEffect(() => {
    ...
    socketRef.current = io(urls.Socket_url);
      // Emit to join room
      socketRef.current.emit('eventBookingSeatLayout', { eventId: id });

      // Listen for updates
      socketRef.current.on('eventSeatLayout', (data:any) => {
  ...
  return (
    <>
    <Breadcrumb pageName="Real-Time Sitting Seat Status" parentName="Event Management" parentPath="/events" />
    <div className="p-5">
...
<div className="flex justify-center">
  <div className="flex gap-4 bg-white shadow-md p-4 rounded-lg border border-gray-100">
    ...
    <div className="bg-green-100 text-green-700 font-semibold text-sm px-4 py-2 rounded border border-green-300">
      ...
      <div className="bg-gray-100 p-5 rounded shadow-lg">{renderSocketSeats()}</div>
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              ...
              >
                IsScanned {renderSortIcon('isScanned')}
              </th>
              <th
                className="min-w-[150px] py-4 px-4 font-bold text-black dark:text-white cursor-pointer text-center whitespace-nowrap"
...
export default EventRealTimeSittingSeatStatus;
