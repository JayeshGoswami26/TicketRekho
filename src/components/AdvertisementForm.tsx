...
interface ModalformProps {
  advertisement?: Advertisement | null; // Make this consistent
  onSubmitSuccess?: (data: Advertisement) => void; // Specify the expected type for data
  onCancel?: () => void; // Include the onCancel prop for the cancel action
}

const AdvertisementForm: React.FC<ModalformProps> = ({
  advertisement,
  onSubmitSuccess,
  onCancel,
}) => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (selectedImage) {
        formData.append('adsImg', selectedImage);
      }

      const requestUrl = advertisement ? urls.updateBannerUrl : urls.createAdvertisementUrl;
      if (advertisement) formData.append('id', advertisement._id);
              ...
              <div className="p-6.5 grid grid-cols-1 gap-x-6">
                ...
                {success && (
                  <p className="text-green-500">
                    Banner {advertisement ? 'updated' : 'created'} successfully!
                  </p>
                )}
              </div>
            </form>
