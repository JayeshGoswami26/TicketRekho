...
const EnquiryTable: React.FC = () => {
  const [sTickets, setsTickets] = useState<EnquiryTicket[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(15);
  const [totalPages, setTotalPages] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');
  const [sortConfig, setSortConfig] = useState<{
    key: keyof EnquiryTicket | null;
    direction: string;
  }>({
    key: null,
    direction: 'ascending',
  ...
  const handleSort = (key: keyof EnquiryTicket) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data && response.data.status) {
          toast.success(
            'Enquiry ticket status changed successfully!',
          );
...
const filteredTickets = sTickets.filter((ticket) => {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (response.data.status) {
        ...
        setsTickets((prevTicket) =>
          prevTicket.filter((ticket) => ticket._id !== id),
        );
      } else {
  ...
  const renderSortIcon = (key: keyof EnquiryTicket) => {
