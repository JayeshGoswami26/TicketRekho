...
const SubTwithdrawTable: React.FC = () => {
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: keyof Manager['sellerId'] | 'amount' | 'status' | 'createdAt' | null;
    direction: string;
  }>({
    key: null,
    direction: 'ascending',
  ...
  const fetchRequests = (
    ...
    const url =
      `${Urls.SubTeamWithdrawalUrl}?page=${page}&limit=${limit}` +
  ...
  // const handleSort = (key: keyof Manager['sellerId']) => {
  //   let direction = 'ascending';
  //   if (sortConfig.key === key && sortConfig.direction === 'ascending') {
  //     direction = 'descending';
  //   }
  //   const sortedRequest = [...request].sort((a, b) => {
  //     if (a.sellerId[key] < b.sellerId[key]) {
  ...
  // };
   ...
   const handleSort = (
     key: keyof Manager['sellerId'] | 'amount' | 'status' | 'createdAt',
   ) => {
     let direction = 'ascending';
     if (sortConfig.key === key && sortConfig.direction === 'ascending') {
       direction = 'descending';
     ...
     const sortedRequest = [...request].sort((a, b) => {
       ...
       if (key === 'amount') {
         ...
         aValue = a.sellerId[key];
         bValue = b.sellerId[key];
       }
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              ...
              >
                Seller Name {renderSortIcon('name')}
              </th>
              <th
              ...
              : filteredRequest.map((req, index) => (
                  <tr key={req._id} className="cursor-pointer">
                    <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11 flex items-center">
                      <img
                        src={`${Urls.Image_url}${req.sellerId.profileImage}`}
                        alt={req.sellerId.name}
...
export default SubTwithdrawTable;
