...
import VenueFormForSitting from '../VenueFormForSitting';
...
const VenueSittingTable: React.FC = () => {
   const { id } = useParams(); 
  ...
  const fetchManagers = (page: number, limit: number) => {
    setLoading(true);
    axios
      .get(`${Urls.displaysittingTickets}?venueId=${id}&page=${page}&limit=${limit}`, {
        headers: {
            ...
            {loading
              ...
              : filteredManagers.map((venue, index) => (
                  <tr key={index} className="cursor-pointer">
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">{coupon.applicableTo}</p>
                    </td> */}
...
export default VenueSittingTable;
